...
// ── Her istekte taze cookie context ──────────────────────
function makeConn(string $wsdl, array $baseOpts, string $certPath, string $sessionId): SoapClient
{
  $ctx = stream_context_create([
    'http' => ['header' => 'Cookie: CSAPSESSIONID=' . $sessionId],
    'ssl' => ['verify_peer' => true, 'verify_peer_name' => true, 'cafile' => $certPath],
  ]);
  return new SoapClient($wsdl, array_merge($baseOpts, ['stream_context' => $ctx]));
}
...
// ────────────────────────────────────────────────────────
// 3. BELGE GÖNDER — taze bağlantı + V_1_3
// ────────────────────────────────────────────────────────
echo "── belgeGonder (V_1_3) ──\n";

// Her istek için taze SoapClient oluştur
$conn = makeConn($connectorWsdl, $soapOpts, $certPath, $sessionId);
...
try {
  $gonderResp = $conn->belgeGonder([
    ...
    'belgeVersiyon' => 'V_1_3',           // ✅ Doğru versiyon
  ]);
