...
<head>
<meta charset="UTF-8">
<title>EK-1 Hurda / Geçici Tutanakları</title>
<style>
...
thead th{background:#7c3aed;color:#fff;padding:10px 12px;text-align:left;font-size:12px;font-weight:600;white-space:nowrap}
...
.badge-hurda{background:#fee2e2;color:#991b1b}
.badge-gecici{background:#fef3c7;color:#92400e}
.badge-tekrar{background:#d1fae5;color:#065f46}
...
.modal-baslik h3{font-size:16px;font-weight:700;color:#7c3aed}
...
.detay-section-baslik{font-size:12px;font-weight:700;color:#7c3aed;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px;padding-bottom:5px;border-bottom:2px solid #ede9fe}
...
.mudahale-badge{display:inline-flex;align-items:center;padding:4px 10px;border-radius:6px;font-size:12px;font-weight:700;margin-bottom:10px}
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px">
  <h2>EK-1: Hurda / Geçici Kullanım Dışı Tutanakları</h2>
  <a href="tutanak_hurda_ekle.php" class="btn btn-blue" style="background:#7c3aed">+ Yeni Tutanak</a>
</div>

<div class="filtre-kart">
  <div class="filtre-grid">
    <div class="f-grup"><label>Arama</label><input type="text" id="aramaInp" placeholder="Satıcı, sicil, marka, müdahale..."></div>
    ...
    <div class="f-grup"><label>Müdahale</label><select id="mudahaleInp">
      <option value="">Tümü</option>
      <option value="hurdaya_ayirma">Hurdaya Ayırma</option>
      <option value="gecici_kullanim_disi">Geçici Kullanım Dışı</option>
      <option value="tekrar_kullanima_alma">Tekrar Kullanıma Alma</option>
    </select></div>
    <div class="f-grup"><label>&nbsp;</label><button class="btn btn-blue" style="background:#7c3aed" onclick="listeyiYukle()">Ara</button></div>
  </div>
</div>

<div class="ozet-grid">
  <div class="ozet-kart"><div class="ozet-sayi" id="ozetToplam">-</div><div class="ozet-etiket">Toplam Tutanak</div></div>
  <div class="ozet-kart"><div class="ozet-sayi" id="ozetHurda" style="color:#dc2626">-</div><div class="ozet-etiket">Hurdaya Ayırma</div></div>
  <div class="ozet-kart"><div class="ozet-sayi" id="ozetGecici" style="color:#d97706">-</div><div class="ozet-etiket">Geçici Kullanım Dışı</div></div>
  <div class="ozet-kart"><div class="ozet-sayi" id="ozetTekrar" style="color:#059669">-</div><div class="ozet-etiket">Tekrar Kullanıma Alma</div></div>
</div>
...
<div class="tablo-wrap">
  <table>
    <thead>
      <tr>
        <th>Sıra No</th><th>Tarih</th><th>Mükellef / Satıcı</th>
        <th>Cihaz</th><th>Sicil No</th><th>Z No / Küm Top</th><th>Müdahale</th><th>Durum</th>
        <th style="min-width:270px">İşlemler</th>
      </tr>
    </thead>
...
<div class="modal-arka" id="detayModal">
  <div class="modal-kutu">
    <div class="modal-baslik"><h3>EK-1 Tutanak — <span id="detaySiraNo"></span></h3><button class="modal-kapat" onclick="kapat('detayModal')">✕</button></div>
...
<div class="modal-arka" id="smsModal">
  <div class="modal-kutu sm">
    ...
    <div class="modal-govde">
      <div style="font-size:12px;font-weight:600;color:#374151;margin-bottom:8px">Alıcıyı Seç:</div>
      <div id="smsSaticiKutu" class="sms-tel-kutu" onclick="smsTelSec(this,'satici')">
        <div class="isim">Mükellef / Satıcı</div><div class="tel" id="smsSaticiTel">—</div>
      </div>
...
<div class="modal-arka" id="silModal">
  <div class="modal-kutu sm">
    ...
    <div class="modal-govde"><p style="font-size:14px">Bu tutanağı silmek istediğinizden emin misiniz?</p></div>
...
let smsTelTipi = 'satici';
let offsetVal = 0;
const LIMIT = 50;

const SMS_SABLON = `Sayın {KISI_ADI},

{SIRA_NO} numaralı EK-1 Tutanağı düzenlenmiştir.
...
const mudahaleMap={hurdaya_ayirma:'Hurdaya Ayırma',gecici_kullanim_disi:'Geçici Kullanım Dışı',tekrar_kullanima_alma:'Tekrar Kullanıma Alma'};
const mudahaleBadge={hurdaya_ayirma:'badge-hurda',gecici_kullanim_disi:'badge-gecici',tekrar_kullanima_alma:'badge-tekrar'};
...
async function listeyiYukle(sifirlaOffset=true){
  ...
  const p={action:'get_hurda_liste',arama:document.getElementById('aramaInp').value.trim(),tarih_bas:document.getElementById('tarihBasInp').value,tarih_bit:document.getElementById('tarihBitInp').value,mudahale_amaci:document.getElementById('mudahaleInp').value,limit:LIMIT,offset:offsetVal};
...
async function ozetYukle(){
  const [rT,rH,rG,rTek]=await Promise.all([api({action:'get_hurda_liste',limit:1,offset:0}),api({action:'get_hurda_liste',limit:1,offset:0,mudahale_amaci:'hurdaya_ayirma'}),api({action:'get_hurda_liste',limit:1,offset:0,mudahale_amaci:'gecici_kullanim_disi'}),api({action:'get_hurda_liste',limit:1,offset:0,mudahale_amaci:'tekrar_kullanima_alma'})]);
  document.getElementById('ozetToplam').textContent=rT.toplam||0;
  document.getElementById('ozetHurda').textContent=rH.toplam||0;
  document.getElementById('ozetGecici').textContent=rG.toplam||0;
  document.getElementById('ozetTekrar').textContent=rTek.toplam||0;
}
...
function renderTablo(data){
  ...
  tb.innerHTML=data.map(d=>`<tr>
    ...
    <td>${fT(d.tarih)}</td>
    <td title="${esc(d.satici_adi)}" style="max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(d.satici_adi)||'—'}</td>
    ...
    <td><span class="badge ${mudahaleBadge[d.mudahale_amaci]||'badge-taslak'}">${mudahaleMap[d.mudahale_amaci]||esc(d.mudahale_amaci)||'—'}</span></td>
    ...
    <td><div class="islem-grup">
      ...
      <a href="tutanak_hurda_yazdir.php?id=${d.id}" target="_blank" class="btn btn-blue btn-sm" title="Yazdır/PDF">🖨️</a>
      ...
      ${d.fatura_id
        ...
        : `<a href="fatura_ekle.php?from_tutanak=hurda&tutanak_id=${d.id}" class="btn btn-green btn-sm" title="Fatura Oluştur">🧾 Fatura</a>`}
      <a href="tutanak_hurda_ekle.php?edit_id=${d.id}" class="btn btn-gray btn-sm" title="Düzenle">✏️</a>
...
async function detayGoster(id){
  const r=await api({action:'get_hurda',id}); if(!r.success) return;
  const d=r.data;
  ...
  const servisAdi=d.yetkili_servis_adi||d.servis_adi||''; const ilkZ=d.kullanim_baslangic_tarihi||d.ilk_z_tarihi||''; const sonZ=d.son_kullanim_tarihi||d.son_z_tarihi||'';
  const mudahaleMetin=mudahaleMap[d.mudahale_amaci]||d.mudahale_amaci||'—';
  document.getElementById('detayIcerik').innerHTML=`
    <div class="detay-section"><div class="detay-section-baslik">Tutanak Bilgileri</div><div class="detay-grid">
      ...
      <div class="detay-alan" style="grid-column:1/-1"><span class="detay-etiket">Müdahale Amacı</span><span class="detay-deger"><span class="badge ${mudahaleBadge[d.mudahale_amaci]||'badge-taslak'}" style="padding:4px 10px">${mudahaleMetin}</span></span></div>
    </div></div>
    ...
    <div class="detay-section"><div class="detay-section-baslik">Mükellef / Satıcı</div><div class="detay-grid">
  ...
  document.getElementById('detayAltlik').innerHTML=`
    ...
    <a href="tutanak_hurda_yazdir.php?id=${d.id}" target="_blank" class="btn btn-blue">🖨️ Yazdır</a>
    <a href="tutanak_hurda_ekle.php?edit_id=${d.id}" class="btn btn-gray">✏️ Düzenle</a>
    ${d.fatura_id
      ? `<span class="btn" style="background:#d1fae5;color:#065f46;cursor:default" title="Fatura No: #${d.fatura_id}">✅ Fatura Oluşturuldu</span>`
      : `<a href="fatura_ekle.php?from_tutanak=hurda&tutanak_id=${d.id}" class="btn btn-green">🧾 Fatura Oluştur</a>`}
  `;
...
async function smsAc(id){
  const r=await api({action:'get_hurda',id}); if(!r.success) return;
  const d=r.data; smsAktifTutanak=d; smsTelTipi='satici';
  const saticiTel=d.satici_tel||'';
  document.getElementById('smsSaticiTel').textContent=saticiTel||'(Telefon girilmemiş)';
  document.getElementById('smsSaticiKutu').classList.add('secili');
  document.getElementById('smsSeciliNumara').textContent=saticiTel||'Seçilmedi';
  const marka=d.cihaz_marka||d.cihaz_marka_adi||''; const model=d.cihaz_model||d.cihaz_model_adi||'';
  const link=BASE_URL+'tutanak_hurda_yazdir.php?id='+id;
  ...
  const mesaj=SMS_SABLON.replace('{KISI_ADI}',d.satici_adi||'Müşterimiz').replace('{SIRA_NO}',d.sira_no||'').replace('{TARIH}',tarihStr).replace('{MUDAHALE}',mudahaleMap[d.mudahale_amaci]||d.mudahale_amaci||'').replace('{MARKA}',marka).replace('{MODEL}',model).replace('{SICIL_NO}',d.cihaz_sicil_no||'').replace('{LINK}',link).replace('{SERVIS_ADI}',servisAdi);
...
function smsTelSec(el,tip){
  ...
  const tel=smsAktifTutanak?.satici_tel||'';
...
function silOnayla(id){
  document.getElementById('silOnayBtn').onclick=async()=>{
    const r=await fetch(BASE,{method:'POST',body:new URLSearchParams({action:'delete_hurda',id})}).then(r=>r.json());
