...
// ────────────────────────────────────────────────────────
// 1. WSDL'deki TÜM metodları listele
// ────────────────────────────────────────────────────────
...
foreach ($conn0->__getFunctions() as $fn) {
  echo "  $fn\n";
}
echo "\n";
try {
...
// ────────────────────────────────────────────────────────
// 2. userService metodlarını da listele
// ────────────────────────────────────────────────────────
[$uc1, $sid1] = freshLogin($userWsdl, $soapOpts, $userId, $password, $lang);
$userConn = new SoapClient($userWsdl, $soapOpts);

echo "── userService Metodları ──\n";
foreach ($userConn->__getFunctions() as $fn) {
  echo "  $fn\n";
}
echo "\n";
try {
  $uc1->logout(['userId' => $userId]);
} catch (SoapFault $e) {
}

// ────────────────────────────────────────────────────────
// 3. belgeTuru olası değerlerini WSDL raw XML'den çek
// ────────────────────────────────────────────────────────
echo "── WSDL Raw'dan belgeTuru Enum Ara ──\n";
$wsdlRaw = file_get_contents(
  $connectorWsdl,
  false,
...
// belgeTuru ile ilgili tüm satırları bul
$lines = explode("\n", $wsdlRaw);
foreach ($lines as $i => $line) {
  if (
    stripos($line, 'belgeTuru') !== false ||
    stripos($line, 'enumeration') !== false ||
    stripos($line, 'EFATURA') !== false ||
    stripos($line, 'EARSIV') !== false ||
    stripos($line, 'IRSALIYE') !== false
  ) {
    echo "  [" . ($i + 1) . "] " . trim($line) . "\n";
  }
}
...
// ────────────────────────────────────────────────────────
// 4. gidenBelgeParametreleri struct'ı ile belgeGonder dene
//    (wsFaturaNo, seriNo, erpKodu ekli)
// ────────────────────────────────────────────────────────
echo "── gidenBelgeParametreleri ile Gönder ──\n";

$turler = ['EFATURA', 'EARSIV', 'EIRSALIYE', 'MUSTAHSIL', 'SERBEST_MESLEK'];
